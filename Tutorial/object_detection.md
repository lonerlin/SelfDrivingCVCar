...
系统封装了Recognition类，实现了目标检测的基本操作，Recognition提供了get_objects用于返回检测到的目标对象的一个Object列表（Object请
...
recognition.close()
```
假设你现在需要判断一个人是否出现在场地中，条件是：人的宽度超过30个像素（重复出现两个人的间隔设定为五秒），你可以修改一下上面的程序：
```python
