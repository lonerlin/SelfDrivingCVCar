...
class FindIntersection:
    ...
    def find(self,image, point, rander_image=None):
        _,scan_data = self.scan_circle(image, point, self.radius, look_angle=self.angle, display_image=rander_image)
        ...
        if len(return_value) > 0 and not (rander_image is None):
            for end_point in return_value:
                cv2.arrowedLine(rander_image, point, (end_point[1], end_point[2]), color=(0, 255, 0), thickness=1)
        return  return_value


if __name__ == '__main__':
    image = cv2.imread('h:/2.jpg')
